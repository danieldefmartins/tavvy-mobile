...
export default function RVCampingBrowseScreen({ navigation, route }: { navigation: any; route?: { params?: { view?: string } } }) {
  ...
  return <SafeAreaView style={[styles.screen,{backgroundColor:theme.background}]} edges={['top']}>
    ...
    {view==='map'?<ScrollView contentContainerStyle={styles.results} keyboardShouldPersistTaps="handled">
      {header}
      <View style={styles.mapOptions}>{chip(copy(locating?'Finding your location…':'My location'),()=>void locate(),false,locating)}{Object.entries(MAP_TILES).map(([id,layer])=><React.Fragment key={id}>{chip(copy(layer.name),()=>setMapStyle(id as keyof typeof MAP_TILES),mapStyle===id)}</React.Fragment>)}</View>
      ...
      <View style={[styles.mapFrame,{borderColor:theme.border,backgroundColor:theme.surface}]}>{catalog.loading?<ActivityIndicator style={{margin:30}} color={theme.primary} accessibilityLabel={copy('Loading places…')}/>:<MapLibreGL.MapView style={{flex:1}} logoEnabled={false} attributionEnabled zoomEnabled scrollEnabled rotateEnabled pitchEnabled>
